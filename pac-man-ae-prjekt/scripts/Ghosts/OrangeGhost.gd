...
func _ready():
	if Global.died_in_level == true: dots_to_leave = dots_to_leave_after_death
...
func start_wave(mode: Mode, duration: float):
	current_mode = mode
	if isReady: wave_timer.start(duration)

func _on_wave_timeout():
	if current_mode == Mode.SCATTER: start_wave(Mode.CHASE, 20.0) 
...
func update_path():
	...
	if is_eaten: current_target = spawnpoint.global_position
	else:
		var distance_to_pacman = global_position.distance_to(pacman.global_position)
		if current_mode == Mode.CHASE:
			if is_panicking:
				if distance_to_pacman > resume_chase_distance: is_panicking = false
			else:
				if distance_to_pacman < panic_distance: is_panicking = true
		else: is_panicking = false
		if current_mode == Mode.SCATTER or is_panicking:
			if global_position.distance_to(current_scatter_target) < 32.0: pick_new_scatter_target()
			current_target = current_scatter_target
		else: current_target = pacman.global_position
	var new_path = main_node.get_path_to_pacman(global_position, current_target)
	if new_path.is_empty() and current_target == current_scatter_target:
		pick_new_scatter_target()
		return 
	if new_path.is_empty() or new_path.size() <= 1: return
	new_path.pop_front()
...
func _process(delta):
	if Global.isIntermissionMode != was_intermission:
		...
		if Global.isIntermissionMode == true: is_immune = false
	if is_in_house:
		if is_game_started and Global.isGameStopped == false: 
			...
			if dots_eaten_since_spawn >= dots_to_leave or current_time_in_house >= time_to_leave: leave_house() 
		return
	if current_path.is_empty(): return
	...
	if global_position.distance_to(target_position) < 1.0:
		...
		if not current_path.is_empty(): set_next_target()
		else:
...
func get_eaten():
	...
	var anims = {200: "Score200", 400: "Score400", 800: "Score800", 1600: "Score1600"}
	if Global.eatGhostScore in anims: $AnimatedSprite2D.play(anims[Global.eatGhostScore])
...
func get_direction():
	if Global.isIntermissionMode and not is_eaten and not is_immune:
		if pacman != null and pacman.intermission_time_left <= pacman.intermission_blink_time: $AnimatedSprite2D.play("ScaredBlink")
		else: $AnimatedSprite2D.play("Scared")
		return
	var direction = target_position - global_position
	if direction.length() < 0.5: return
	if abs(direction.x) > abs(direction.y):
		if direction.x > 0: $AnimatedSprite2D.play("Augen_Rechts" if is_eaten else "Rechts")
		else: $AnimatedSprite2D.play("Augen_Links" if is_eaten else "Links")
	else:
		if direction.y > 0: $AnimatedSprite2D.play("Augen_Unten" if is_eaten else "Unten")
		else: $AnimatedSprite2D.play("Augen_Oben" if is_eaten else "Oben")
...
func reset_ghost():
	...
	if dots_to_leave <= 0: leave_house()
