...
func _on_wave_timeout():
	if current_mode == Mode.SCATTER: start_wave(Mode.CHASE, 15.0) 
...
func update_path():
	...
	if is_eaten: current_target = spawnpoint.global_position
		...
		if current_mode == Mode.CHASE:
			...
			if "direction" in pacman: pacman_direction = pacman.direction.normalized()
			elif "velocity" in pacman and pacman.velocity.length() > 0: pacman_direction = pacman.velocity.normalized()
